...
public class AverageSavesCalculator(IPlayerPerformanceRepository playerPerformanceRepository) : IPerformanceCalculator
{
    public async Task<double> Calculate(string PlayerId, int NumberOfWeeks)
    {
        ...
        return playerPerformanceEntities.Average(p => p.Stats.Saves);
    }
}